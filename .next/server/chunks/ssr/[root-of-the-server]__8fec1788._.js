...
function AdminPage() {
    ...
    const handleAddVendor = (e)=>{
        ...
        const newVendor = {
            id: (vendors.length > 1 ? Math.max(...vendors.map((v)=>parseInt(v.id))) + 1 : 1).toString(),
            name: newVendorName,
        ...
        toast({
            title: "店家已新增",
            description: `${newVendorName} 已成功加入店家列表。`
        });
    };
    const handleDeleteVendor = (vendorId, vendorName)=>{
        ...
        toast({
            title: "店家已刪除",
            description: `${vendorName} 已從列表中移除。`
        });
    };
...
//# sourceMappingURL=%5Broot-of-the-server%5D__8fec1788._.js.map